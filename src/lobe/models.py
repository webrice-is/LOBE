...
class VerifierProgression(BaseModel, db.Model):
    ...
    owned_icons = db.relationship("VerifierIcon", secondary=progression_icon)
    owned_titles = db.relationship("VerifierTitle", secondary=progression_title)
    owned_quotes = db.relationship("VerifierQuote", secondary=progression_quote)
    owned_fonts = db.relationship("VerifierFont", secondary=progression_font)
...
class VerifierIcon(BaseModel, db.Model):
    ...
    title = db.Column(
        db.String(64),
        info={"validators": [validators.InputRequired()], "label": "Titill"},
    )
    description = db.Column(
        db.String(255),
        info={"validators": [validators.InputRequired()], "label": "Stutt lýsing"},
    )
    price = db.Column(db.Integer(), default=0, info={"label": "Verð", "min": 0, "max": 1000})
    color = db.Column(
        db.String(),
        info={
            "label": "Litur á merki",
    ...
    rarity = db.Column(
        ...
        info={
            "validators": [validators.InputRequired()],
            "label": "Tegund",
            "choices": [(0, "Basic"), (1, "Rare"), (2, "Epic"), (3, "Legendary")],
        },
    )
...
class VerifierTitle(BaseModel, db.Model):
    id = db.Column(db.Integer(), primary_key=True)
    title = db.Column(db.String(64), info={"label": "Titillinn"})
    description = db.Column(db.String(255), info={"label": "Stutt lýsing"})
    price = db.Column(db.Integer(), default=0, info={"label": "Verð"})
    rarity = db.Column(
        db.Integer(),
        info={
            "validators": [validators.InputRequired()],
...
class VerifierQuote(BaseModel, db.Model):
    id = db.Column(db.Integer(), primary_key=True)
    quote = db.Column(db.String(255), info={"label": "Slagorðið"})
    price = db.Column(db.Integer(), default=0, info={"label": "Verð"})
    rarity = db.Column(
        db.Integer(),
        info={
...
class VerifierFont(BaseModel, db.Model):
    ...
    font_family = db.Column(
        db.String(),
        info={"label": "Font fjölskylda", "description": "Til dæmis Megrim"},
    )
    font_type = db.Column(db.String(), info={"label": "Font týpa", "description": "Til dæmis cursive"})
    href = db.Column(db.String(), info={"label": "Linkur á font CSS"})
    title = db.Column(db.String(255), info={"label": "Titill"})
    description = db.Column(db.String(255), info={"label": "Lýsing"})
    price = db.Column(db.Integer(), default=0, info={"label": "Verð (gimsteinar)"})
    rarity = db.Column(
        db.Integer(),
        info={
            "validators": [validators.InputRequired()],
...
class PremiumItem(BaseModel, db.Model):
    ...
    title = db.Column(db.String(255), info={"label": "Titill"})
    description = db.Column(db.String(255), info={"label": "Lýsing"})
    coin_price = db.Column(db.Integer(), default=0, info={"label": "Verð (aurar)"})
    experience_price = db.Column(db.Integer(), default=0, info={"label": "Verð (demantar)"})
    num_available = db.Column(db.Integer(), default=0, info={"label": "Fjöldi í boði"})
    wheel_modifier = db.Column(
        db.Boolean(),
        default=False,
        info={"label": "Breyta þessi verðlaun lukkuhjólinu?"},
    )
    for_sale = db.Column(db.Boolean(), default=True, info={"label": "Til sölu"})
...
class Application(BaseModel, db.Model):
    ...
    def recordings(self):
        return Recording.query.filter(Recording.user_id == self.user_id)

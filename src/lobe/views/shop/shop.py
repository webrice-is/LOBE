...
def random_equip():
    ...
    if all(
        ...
        flash("Þú verður að eiga a.m.k. tvö stykki af hverri tegund", category="warning")
...
def loot_box(rarity):
    ...
    if set_price <= current_user.progression.lobe_coins:
        ...
        selected_items = list(
            np.random.choice(
                all_items,
                app.config["ECONOMY"]["loot_boxes"]["num_items"] - 1,
        ...
        loot_box_message = json.dumps(
            {str(i): {"type": types[i], "id": item.id} for i, item in enumerate(selected_items)}
        )

...
def lobe_feed():
    ...
    verifiers = sorted(get_verifiers_and_admins(), key=lambda v: -v.progression.weekly_verifies)
        ...
        if current_user.progression.experience >= app.config["ECONOMY"]["social_feed"]["post_youtube"]:
            try:
                ...
                if video_id:
                    ...
                    flash(
                        "Hlekkur ekki á réttu sniði, aðeins youtube myndbönd passa hér",
                        category="warning",
                    )
                    return redirect(url_for("feed.lobe_feed"))
            except Exception as error:
                app.logger.error("Error posting link: {}\n{}".format(error, traceback.format_exc()))
                flash("Villa kom upp við að hengja hlekk upp", category="warning")
        else:
...
def send_banner():
    try:
        return send_from_directory(app.config["STATIC_DATA_DIR"], "banner.png")
    except Exception as error:
        app.logger.error("Error sending a beernight image : {}\n{}".format(error, traceback.format_exc()))
    return ""

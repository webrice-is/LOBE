...
def postings():
    page = int(request.args.get("page", 1))
    postings = Posting.query.order_by(
        resolve_order(
            Posting,
            request.args.get("sort_by", default="created_at"),
            order=request.args.get("order", default="desc"),
...
def posting_detail(id):
    ...
    return render_template("posting.jinja", posting=posting, info=info, section="posting")
...
def new_application(posting_uuid):
        ...
        if form.validate():
            ...
            if not user:
                try:
                    ...
                    return redirect(url_for("application.new_application", posting_uuid=posting_uuid))
            application.user_id = user.id
            db.session.add(application)
            db.session.commit()
            return redirect(
                url_for("recording.record_session", collection_id=posting.collection) + f"?user_id={user.id}"
            )
    ...
    return render_template(
        ...
        type="create",
        posting=posting,
        action=url_for("application.new_application", posting_uuid=posting_uuid),
    )

...
class IndexManager:
    ...
    def add(self, recording, token, user_name):
        self.index.write("{}\t{}\t{}\n".format(user_name, recording.get_zip_fname(), token.get_fname()))
...
def create_collection_zip(id):
    ...
    try:
        ...
        app.logger.error("Error creating a collection .zip : {}\n{}".format(error, traceback.format_exc()))
    finally:
        zip_manager.close()
        try:
        ...
        for token in tokens:
            for recording in token.recordings:
                if trim_type == 0 and not recording.has_trim or trim_type == 1:
                    ...
                    stamps = find_segment(sample, sr, top_db=collection.configuration.trim_threshold)
